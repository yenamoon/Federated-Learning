...
def run_fl_server(args):
    ...
    data_loader = loader(args.dataset, batch_size=args.batch_size)
    ...
    for e in range(args.epoch):
        round_start = time.time()  # ✅ 라운드 시작 시간
        print('\n================== Epoch {:>3} =================='.format(e + 1))
        ...
        grads_info = []
        for conn in client_sockets:
            client_grad = recv_msg(conn)
            grads_info.append(client_grad)

        s.aggregate_network_mode(grads_info)

        round_end = time.time()  # ✅ 라운드 종료 시간
...
def run_fl_client(args):
    ...
    for e in range(args.epoch):
        ...
        local_grads = c.run_network_mode(global_state)
        send_msg(client_socket, local_grads)

    client_socket.close()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Distributed FedSGD with Docker")
