...
def recv_from_client(conn, clients_info, timeout, lock, timed_out_conns):
    try:
        ...
        print(f'[Server] timeout 초과 → 해당 클라이언트 이번 라운드 제외')
        with lock:
            timed_out_conns.append(conn)
...
def run_fl_server(args):
    ...
    for e in range(args.epoch):
        ...
        # ✅ 글로벌 모델 전송 시 끊어진 소켓 처리
        for conn in client_sockets[:]:
            try:
                send_msg(conn, {'global_state': global_state_cpu, 'timeout': args.timeout})
        ...
        for conn in client_sockets:
            t = threading.Thread(
                ...
                args=(conn, clients_info, args.timeout, lock, timed_out_conns)
            )
            threads.append(t)
            t.start()
        ...
        # ✅ timeout 발생한 클라이언트에게 SKIP 신호 전송
        for conn in timed_out_conns:
            try:
                send_msg(conn, 'SKIP')
                print(f'[Server] SKIP 신호 전송')
            except BrokenPipeError:
                print(f'[Server] SKIP 전송 실패 → 연결 끊김')
                if conn in client_sockets:
                    client_sockets.remove(conn)
            except Exception as e:
                print(f'[Server] SKIP 신호 전송 오류: {e}')
...
def run_fl_client(args):
    ...
    for e in range(args.epoch):
        # 1. 서버로부터 글로벌 모델 + timeout 또는 SKIP 수신
        msg = recv_msg(client_socket)
        if msg is None:
            break

        # ✅ SKIP 신호 수신 시 다음 라운드 대기
        if msg == 'SKIP':
            print(f'[Client {args.rank}] SKIP 신호 수신 → 다음 라운드 대기')
            continue

        global_state = msg['global_state']
        timeout = msg['timeout']

        # 2. 로컬 학습 (매 epoch마다 timeout 체크)
        local_info = c.run_network_mode(global_state, client_socket, timeout)

        # ✅ None이면 timeout으로 중단된 것 → SKIP 신호 대기
        if local_info is None:
            print(f'[Client {args.rank}] timeout으로 학습 중단 → SKIP 신호 대기')
            skip_msg = recv_msg(client_socket)
            if skip_msg == 'SKIP':
                print(f'[Client {args.rank}] SKIP 확인 → 다음 라운드 대기')
            continue
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Distributed FedAvg with Timeout")
