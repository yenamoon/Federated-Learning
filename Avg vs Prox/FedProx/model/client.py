...
class client(object):
    ...
    def __train(self, model, global_model, start_time, timeout):
        ...
        for e in range(self.local_epoch):
            ...
            for data, target in self.train_loader:
                ...
                if elapsed >= timeout:
                    print(f'[Rank {self.rank}] timeout 감지 ({elapsed:.1f}초 경과, '
                          f'epoch {e+1} 진행 중 중단)')
                    print(f'[Rank {self.rank}] → {completed_epochs}epoch 가중치 전송 (FedProx partial work)')
            ...
            if elapsed >= timeout:
                print(f'[Rank {self.rank}] timeout 감지 ({elapsed:.1f}초 경과, '
                      f'{completed_epochs}/{self.local_epoch} epoch 완료)')
                print(f'[Rank {self.rank}] → 불완전한 가중치 전송 (FedProx partial work)')
                break
